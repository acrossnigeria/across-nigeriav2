...
const GoldAdvertComponent = ( { isLoading, adverts } ) => {
            ...
            { (!isLoading) &&  (
                <div className='h-[200px] md:h-[300px] flex-row flex justify-between md:w-[95%] mx-auto w-[100%] mt-[50px] mb-[50px]'>
                        { ( staticAdvert?.contactUsButton?.showContactButton) && (
                                <div className="h-[100%] w-[49%] flex flex-col border-1 justify-evenly items-center border-gray-400 bg-gray-200 rounded-[5px]">
                                    <BearSlideImage style={{ height:'85%', width:'99%', borderRadius:'5px'}} imageUrl={staticAdvert?.advertImage} />
                                    <div className="flex flex-row justify-between md:px-3 w-[98%] h-[12%] items-center">
                                        ...
                                        <button onClick={()=>{contactOnWhatsApp(staticAdvert?.contactUsButton?.contact)}} className="h-[99%] w-[fit] px-[5px] md:text-[18px] text-[13px] flex flex-row justify-center items-center gap-2 border-1 hover:bg-gray-300 transition-all ease-in-out duration-300 text-gray-500 border-gray-400 rounded-[5px]">
...
export default GoldAdvertComponent;