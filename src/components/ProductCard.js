...
const ProductCard = ( { title, image, link, date, isOpen } ) => {
  ...
  return (
    <div className={' w-[100%] md:w-[300px] flex flex-col h-fit bg-transparent-to-br relative rounded-[5px]'}>
      ...
      <div className='h-[140px] absolute mt-[140px] rounded-tl-[10px] rounded-br-[10px] shadow-xl bg-white w-full flex flex-col items-center text-center justify-between pt-3' >
        ...
        <Link href={link} className={`${isOpen?'bg-green-600 hover:bg-green-500':'bg-gray-300 hover:cursor-default'} h-[45px] flex justify-center items-center text-[18px] w-[100%] rounded-tl-[10px] rounded-br-[10px] text-white`}>Join</Link>
      </div>
    </div>
  );
