...
import { Bell, ChevronDown, Home, Layers, Lock, Megaphone, Menu, PanelLeftClose, PanelLeftOpen, UsersIcon, X } from "lucide-react";
...
const AdminMainPage = () => {
    ...
    if (isAuthenticating) {
        return (
            <div style={{minHeight:`calc(var(--vh, 1vh)*100)`,}} className="w-screen flex flex-col justify-center items-center">
                <Lock strokeWidth={1.5} size={30} color="red"/>
                <div className="flex flex-row items-center gap-1">
                    <ProcessLoader size={'25px'} className={'mt-2'} color={'black'}/>
                ...
                <div className={`${isPanelOpen ? 'w-[calc(100%-290px)] ml-[290px]' : 'w-[calc(100%-70px)] ml-[70px]'} p-2 transition-all duration-300 ease-in-out h-full`}>
                    <div className={`flex h-[50px] flex-row justify-between items-center`}>
                        ...
                        <div style={{alignItems:'center'}} className="flex w-fit flex-row justify-between gap-2 pl-3 pr-[2px] cursor-pointer transition duration-100 text-[16px]">
                            <Link href={'/user/profile'}>Welcome, {session.user?.name}</Link>
